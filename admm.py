...
class ADMM:
    ...
    def prune_z(self, z, imp_score, opt):
        ...
        # z_update = self.metrics_imp_score(z, imp_score, opt)  # 用imp_score
        return z_update
    ...
    def adjust_rho(
        self, epoch, epochs, factor=5
    ):  # 根据训练的当前进度（epoch 和 epochs）调整 rho 值。通常，rho 会随着训练的进展而增大，从而增加对约束的惩罚
    ...
    def metrics_sample(
        self, z, opt
    ):  # 该方法根据透明度值的相对权重进行随机采样。首先，将透明度值归一化为概率分布，并按此分布随机选择样本。
    ...
    def metrics_imp_score(
        self, z, imp_score, opt
    ):  # 该方法基于重要性分数（imp_score）更新 z。重要性分数低于某个阈值的透明度值会被置为 0，从而对重要性较低的部分进行稀疏化。
