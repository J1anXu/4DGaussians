...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        self.image_paths, self.mark_pixels_image_paths, self.image_poses, self.image_times, N_cam, N_time = (
            self.load_images_path(videos, self.split)
        )
    ...
    def load_images_path(self, videos, split):
        ...
        for index, video_path in enumerate(videos):
            ...
            video_images_path = video_path.split(".")[0]
            image_path = os.path.join(video_images_path, "images")
            mark_pixels_image_path = video_images_path + "_mark_pixels"
            video_frames = cv2.VideoCapture(video_path)
            if not os.path.exists(image_path):
                ...
                while video_frames.isOpened():
                    ...
                    if ret:
                        ...
                        break
            if not os.path.exists(mark_pixels_image_path):
                print(f"no path in {mark_pixels_image_path}, make empty dirs")
                os.makedirs(mark_pixels_image_path)

            image_names = os.listdir(image_path)
            mark_pixels_image_names = os.listdir(mark_pixels_image_path)
            image_names.sort()
            this_count = 0
            ...
            this_count = 0
            for idx, mark_pixels_image_name in enumerate(mark_pixels_image_names):
                if this_count >= countss:
                    break
                mark_pixels_image_paths.append(os.path.join(mark_pixels_image_path, mark_pixels_image_name))
                this_count += 1

            N_time = len(image_names)

            #     video_data_save[count] = img.permute(1,2,0)
            #     count += 1
        return image_paths, mark_pixels_image_paths, image_poses, image_times, N_cams, N_time
    ...
    def __getitem__(self, index):
        ...
        if self.mark_pixels_image_paths is not None and len(self.mark_pixels_image_paths) > 0:
            mark_pixels_image = Image.open(self.mark_pixels_image_paths[index])
            mark_pixels_image = mark_pixels_image.resize(self.img_wh, Image.LANCZOS)
            mark_pixels_image = self.transform(mark_pixels_image)

        return img, mark_pixels_image, self.image_poses[index], self.image_times[index]
