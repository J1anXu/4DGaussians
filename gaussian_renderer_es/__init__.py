...
import torch
import math
from diff_gaussian_rasterization_es import GaussianRasterizationSettings, GaussianRasterizer
from diff_gaussian_rasterization_es import GaussianRasterizationWithErrorScoreSettings, GaussianWithErrorScoreRasterizer
...
def render_with_error_scores(
    ...
    if cam_type != "PanopticSports":
        tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)
        tanfovy = math.tan(viewpoint_camera.FoVy * 0.5)
        raster_settings = GaussianRasterizationWithErrorScoreSettings(
            image_height=int(viewpoint_camera.image_height),
            image_width=int(viewpoint_camera.image_width),
            tanfovx=tanfovx,
            tanfovy=tanfovy,
    ...
    rasterizer = GaussianWithErrorScoreRasterizer(raster_settings=raster_settings)
