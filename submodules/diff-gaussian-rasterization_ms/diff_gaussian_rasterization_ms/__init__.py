...
def rasterize_gaussians_with_error_score(
    means3D,
    means2D,
    sh,
    colors_precomp,
    ...
    return _RasterizeGaussiansWithErrorScore.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
...
def rasterize_gaussians_depth(
    ...
    return _RasterizeGaussiansWithErrorScore.render_depth(
        means3D,
        means2D,
        sh,
        colors_precomp,
...
class _RasterizeGaussiansWithErrorScore(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means3D,
        ...
        if raster_settings.debug:
            ...
            try:
                (
                    ...
                    binningBuffer,
                    imgBuffer, scores
                ) = _C.rasterize_gaussians_with_score(*args)
...
class GaussianWithErrorScoreRasterizer(nn.Module):
    ...
    def forward(
        ...
        return rasterize_gaussians_with_error_score(
            means3D, means2D, shs, colors_precomp, opacities, scales, rotations, cov3D_precomp, raster_settings
        )

class GaussianRasterizationWithErrorScoreSettings(NamedTuple):
    image_height: int
    image_width: int
    tanfovx: float
    tanfovy: float
